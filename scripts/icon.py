...
downloadPath = os.path.abspath('./../database/icon')
...
url = "https://www.b3.com.br/pt_br/market-data-e-indices/indices/indices-de-segmentos-e-setoriais/indice-consumo-icon-estatisticas-historicas.htm"
...
driver.quit()
if(os.path.exists(os.path.join(downloadPath, "icon.csv"))):
    os.remove(os.path.join(downloadPath, "icon.csv"))
...
for file in os.listdir(downloadPath):
    if file.endswith(".csv"):
        ...
        finalDf.to_csv(os.path.join(downloadPath, "icon.csv"),  sep = ",", index=False)
