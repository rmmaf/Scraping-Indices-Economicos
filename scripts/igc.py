...
downloadPath = os.path.abspath('./../database/IGC')
...
url = "https://www.b3.com.br/pt_br/market-data-e-indices/indices/indices-de-governanca/indice-de-acoes-com-governanca-corporativa-diferenciada-igcx-estatisticas-historicas.htm"
...
driver.quit()
if(os.path.exists(os.path.join(downloadPath, "IGC.csv"))):
    os.remove(os.path.join(downloadPath, "IGC.csv"))
...
for file in os.listdir(downloadPath):
    if file.endswith(".csv"):
        ...
        finalDf.to_csv(os.path.join(downloadPath, "IGC.csv"),  sep = ",", index=False)
